...
class ThreeStreamAttnModel(nn.Module):
    def __init__(self, num_classes, dropout=0.4):
        ...
        # Attention scoring network (small capacity: 1536 * 3 + 3 = 4611 parameters)
        self.attention = nn.Linear(fused_dim, 3)
        
        # Classifier shrinks: takes 512 dimensions (weighted sum) instead of 1536
    ...
    def forward(self, rgb, flow, depth):
        ...
        fused_concat = torch.cat([rgb_feat, flow_feat, depth_feat], dim=1)
        
        # Compute weights: shape (B, 3)
        attn_weights = F.softmax(self.attention(fused_concat), dim=1)
        
        w_rgb = attn_weights[:, 0].unsqueeze(1)
        w_flow = attn_weights[:, 1].unsqueeze(1)
        w_depth = attn_weights[:, 2].unsqueeze(1)
        
        # Weighted sum: shape (B, 512)
        fused_weighted = (w_rgb * rgb_feat) + (w_flow * flow_feat) + (w_depth * depth_feat)
...
def plot_learning_curves(history, fold):
    ...
    plt.tight_layout()
    plt.savefig(METRICS_DIR / f"exp_g_fold_{fold}_learning_curves.png", dpi=150)
    plt.close()
...
def plot_confusion_matrix(cm, class_names, fold):
    ...
    plt.tight_layout()
    plt.savefig(METRICS_DIR / f"exp_g_fold_{fold}_cm.png", dpi=150)
    plt.close()
...
def run_one_fold(fold, df, label_map, epochs, overfit_test=False):
    ...
    model = ThreeStreamAttnModel(num_classes=len(label_map), dropout=0.4).to(DEVICE)
    ...
    patience = 8
    patience_counter = 0
    ckpt_path = Path(f"best_threestream_attn_fold{fold}.pt") if not overfit_test else None
    ...
    if not overfit_test:
        pd.DataFrame(metrics_log).to_csv(METRICS_DIR / f"exp_g_fold_{fold}_metrics.csv", index=False)
...
def main():
    ...
    if args.overfit_test:
        print("\n=== OVERFIT TEST MODE (Experiment G: Attention Fusion) ===\n")
        run_one_fold(0, df, label_map, epochs=args.epochs or 40, overfit_test=True)
        return

    if args.all_folds:
        print("\n=== ALL FOLDS (Experiment G: Attention Fusion) ===\n")
        results = {}
        ...
        print(f"\n{'='*60}\nFINAL SUMMARY (Experiment G: Attention Fusion)\n{'='*60}")
        ...
        print(f"\n=== FOLD {args.fold} (Experiment G: Attention Fusion) ===\n")
