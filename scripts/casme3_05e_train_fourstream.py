...
EXTRACTED_ROIS_DIR = Path("CASME3_Dataset/extracted_rois_v3")
...
class FourStreamDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 2. Eyebrow flow feature
        eyebrow_npz = EXTRACTED_ROIS_DIR / clip_name / "eyebrows.npz"
        try:
            eye_data = np.load(eyebrow_npz)
            eyebrow_flow = eye_data["flow"].copy()
            eyebrow_flow = np.clip(eyebrow_flow / 15.0, -1.0, 1.0)
            # Resize eyebrow flow to 224x224 to feed into ResNet18
            eyebrow_flow = cv2.resize(eyebrow_flow, (224, 224), interpolation=cv2.INTER_LINEAR)
        except Exception as e:
            # Fallback if missing (shouldn't happen since we filtered)
            eyebrow_flow = np.zeros((224, 224, 2), dtype=np.float32)
        ...
        if self.is_train:
            ...
            if random.random() > 0.5:
                ...
                eyebrow_flow = np.flip(eyebrow_flow, axis=1).copy()
                eyebrow_flow[..., 0] = -eyebrow_flow[..., 0]
        ...
        eyebrow_t = torch.from_numpy(eyebrow_flow).float().permute(2, 0, 1)

        label = self.label_map[str(row["emotion"]).strip().lower()]
        return rgb_t, flow_t, depth_t, eyebrow_t, torch.tensor(label, dtype=torch.long)

def build_dataloader(df, label_map, batch_size=16, shuffle=True, is_train=False, use_weighted_sampler=True):
    dataset = FourStreamDataset(df, label_map, is_train=is_train)
    sampler = None
...
class FourStreamModel(nn.Module):
    def __init__(self, num_classes, dropout=0.4):
        ...
        self.eyebrow_backbone, eye_dim = make_backbone(2) # Eyebrow is 2-channel flow

        fused_dim = rgb_dim + flow_dim + depth_dim + eye_dim # 512 * 4 = 2048
        ...
        self._freeze_backbone(self.eyebrow_backbone)
    ...
    def forward(self, rgb, flow, depth, eyebrow):
        rgb_feat = self.rgb_backbone(rgb)
        flow_feat = self.flow_backbone(flow)
        depth_feat = self.depth_backbone(depth)
        eyebrow_feat = self.eyebrow_backbone(eyebrow)
        
        fused = torch.cat([rgb_feat, flow_feat, depth_feat, eyebrow_feat], dim=1)
...
def train_epoch(model, loader, criterion, optimizer):
    ...
    for rgb, flow, depth, eyebrow, y in tqdm(loader, desc="  train", leave=False):
        rgb, flow, depth, eyebrow, y = rgb.to(DEVICE), flow.to(DEVICE), depth.to(DEVICE), eyebrow.to(DEVICE), y.to(DEVICE)
        optimizer.zero_grad()
        logits = model(rgb, flow, depth, eyebrow)
...
def validate(model, loader, criterion):
    ...
    for rgb, flow, depth, eyebrow, y in tqdm(loader, desc="  val  ", leave=False):
        rgb, flow, depth, eyebrow, y = rgb.to(DEVICE), flow.to(DEVICE), depth.to(DEVICE), eyebrow.to(DEVICE), y.to(DEVICE)
        logits = model(rgb, flow, depth, eyebrow)
        loss = criterion(logits, y)
        total_loss += loss.item() * y.size(0)
...
def run_one_fold(fold, df, label_map, epochs, overfit_test=False):
    ...
    model = FourStreamModel(num_classes=len(label_map), dropout=0.4).to(DEVICE)
...
def main():
    ...
    # Ensure BOTH full-face npz and eyebrow npz exist
    df = df[df['clip_name'].apply(lambda x: (PROCESSED_DIR / f"{x}.npz").exists() and (EXTRACTED_ROIS_DIR / x / "eyebrows.npz").exists())].copy()
    ...
    if args.overfit_test:
        print("\n=== OVERFIT TEST MODE (Experiment I: 4-Stream Eyebrow) ===\n")
        run_one_fold(0, df, label_map, epochs=args.epochs or 40, overfit_test=True)
        return

    if args.all_folds:
        print("\n=== ALL FOLDS (Experiment I: 4-Stream Eyebrow) ===\n")
        results = {}
        ...
        print(f"\n{'='*60}\nFINAL SUMMARY (Experiment I: 4-Stream Eyebrow)\n{'='*60}")
