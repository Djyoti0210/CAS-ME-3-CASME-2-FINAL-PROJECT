...
def run_one_fold(fold, df, label_map, epochs, overfit_test=False):
    ...
    patience = 8
    patience_counter = 0
    ckpt_path = Path(f"casme3_best_dualflow_fold{fold}.pt") if not overfit_test else None
    ...
    if not overfit_test:
        pd.DataFrame(metrics_log).to_csv(METRICS_DIR / f"exp_j_dualflow_fold_{fold}_metrics.csv", index=False)
