...
# --- PURE CAS(ME)3 MODEL DEFINITION (Path A: ResNet18 x3) ---
...
class PureFusionModel(nn.Module):
    def __init__(self, num_classes, dropout=0.5):
        super().__init__()
        # perfectly matched ImageNet ResNet18s to avoid Domain Shift
        ...
        # Freezing Discipline: Only Layer 4 unfreezes
        self._freeze_resnet_backbone(self.rgb_backbone)
    ...
    def forward(self, rgb, flow, depth):
        ...
        if self.training:
            ...
            # 2. Modality Dropout (Cures Modality Starvation)
            p_drop = 0.2
            mask_r = (torch.rand(r.size(0), 1, device=r.device) > p_drop).float()
            mask_f = (torch.rand(f.size(0), 1, device=f.device) > p_drop).float()
            mask_d = (torch.rand(d.size(0), 1, device=d.device) > p_drop).float()
            
            r = r * mask_r / (1.0 - p_drop)
            f = f * mask_f / (1.0 - p_drop)
            d = d * mask_d / (1.0 - p_drop)
...
# --- MATHEMATICALLY CORRECT FOCAL LOSS ---
...
class FocalLoss(nn.Module):
    ...
    def forward(self, logits, targets):
        # 1. Pure Cross Entropy to get accurate `pt` (probability of true class)
        ce_loss_pure = F.cross_entropy(logits, targets, weight=self.alpha, reduction='none')
        pt = torch.exp(-ce_loss_pure)
        
        # 2. Smoothed Cross Entropy for the actual gradient target
        ce_loss_smooth = F.cross_entropy(logits, targets, weight=self.alpha, reduction='none', label_smoothing=self.smoothing)
        
        # 3. Combine them. Standard F.cross_entropy with label smoothing breaks Focal Loss mathematically. This fixes it.
        focal_loss = ((1 - pt) ** self.gamma) * ce_loss_smooth
...
def plot_learning_curves(history, fold):
    ...
    plt.tight_layout()
    plt.savefig(METRICS_DIR / f"fold_{fold}_pure_learning_curves.png", dpi=150)
    plt.close()
...
def plot_confusion_matrix(cm, class_names, fold):
    ...
    plt.tight_layout()
    plt.savefig(METRICS_DIR / f"fold_{fold}_pure_cm.png", dpi=150)
    plt.close()
...
def run_one_fold(fold, df, label_map, epochs):
    ...
    model = PureFusionModel(num_classes=len(label_map), dropout=0.5).to(DEVICE)
    
    class_weights = compute_class_weights(train_df, label_map, DEVICE)
    criterion = FocalLoss(alpha=class_weights, gamma=2.0, label_smoothing=0.15)
        
    # Differential LR: Let the fusion head learn 5x faster than the backbones
    head_params = []
    backbone_params = []
    for name, param in model.named_parameters():
        if not param.requires_grad: continue
        if "classifier" in name or "attn" in name:
    ...
    patience = 12
    patience_counter = 0
    ckpt_path = Path(f"casme3_best_threestream_pure_fold{fold}.pt")
    ...
    pd.DataFrame(metrics_log).to_csv(METRICS_DIR / f"fold_{fold}_pure_metrics.csv", index=False)
...
def main():
    ...
    if args.all_folds:
        print("\n=== ALL FOLDS PURE CAS(ME)3 FUSION (Focal Bug Fixed) ===\n")
        results = {}
