"""
04_train_twostream.py -- Stage 3: RGB + Flow two-stream model training.
Run --overfit_test FIRST to confirm the new architecture can learn, exactly
like Stage 2. Only proceed to --all_folds once that passes.
"""
import argparse
...
def run_one_fold(fold, df, label_map, epochs, overfit_test=False):
    ...
    model = TwoStreamModel(num_classes=len(label_map), dropout=0.5).to(DEVICE)
    ...
    patience = 8
    patience_counter = 0
    ckpt_path = Path(f"best_twostream_fold{fold}.pt") if not overfit_test else None
...
def main():
    ...
    if args.all_folds:
        ...
        print(f"\n=== FOLD {args.fold} (Two-Stream: RGB+Flow) ===\n")
