"""
06_train_temporal.py -- Trains the CNN+GRU temporal model on full frame
sequences. Run --overfit_test FIRST, same discipline as every prior stage.
"""
import argparse
...
from dataset_temporal import fit_label_map, build_dataloader
from model_temporal import CNNGRUModel
...
def run_one_fold(fold, df, label_map, epochs, overfit_test=False):
    if overfit_test:
        ...
        train_loader = build_dataloader(tiny_df, label_map, batch_size=4, shuffle=True,
        ...
        train_loader = build_dataloader(train_df, label_map, batch_size=8, shuffle=True,
                                         is_train=True, use_weighted_sampler=False)
        val_loader = build_dataloader(val_df, label_map, batch_size=8, shuffle=False, is_train=False)
        lr = 1e-4

    model = CNNGRUModel(num_classes=len(label_map), dropout=0.5).to(DEVICE)
    if overfit_test:
    ...
    patience = 8
    patience_counter = 0
    ckpt_path = Path(f"best_temporal_fold{fold}.pt") if not overfit_test else None
...
def main():
    ...
    if args.all_folds:
        ...
        print(f"\n=== FOLD {args.fold} (CNN+GRU Temporal) ===\n")
