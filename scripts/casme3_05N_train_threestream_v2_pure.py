...
class PureFusionModel(nn.Module):
    def __init__(self, num_classes, dropout=0.5):
        super().__init__()
        # ALL streams use ResNet18 so they are perfectly matched!
    ...
    def forward(self, rgb, flow, depth):
        ...
        if self.training:
            # 1. FEATURE NOISE: Artificially multiplies dataset size
            r = r + torch.randn_like(r) * 0.05
            f = f + torch.randn_like(f) * 0.05
            d = d + torch.randn_like(d) * 0.05
            
            # 2. MODALITY DROPOUT (The 0.4 Secret Weapon)
            # Randomly black out one of the streams with 20% probability. 
            # This ruthlessly forces the network to learn from ALL streams and cures Modality Starvation!
            p_drop = 0.2
...
def main():
    ...
    if args.all_folds:
        print("\n=== ALL FOLDS PURE CAS(ME)3 FUSION (No Domain Shift) ===\n")
        results = {}
